...
class A2C:
    ...
    def update(
        ...
        # Handle both recurrent and non-recurrent cases
        # ============================================================
        if hidden_state.dim() == 3:
            # Recurrent case: (num_steps, num_processes, hidden_dim)
            hidden_for_lambda = hidden_state[0, 0, :].unsqueeze(0)
        elif hidden_state.dim() == 2:
            # Non-recurrent case: (num_processes * num_steps, hidden_dim)
            hidden_for_lambda = hidden_state[0, :].unsqueeze(0)
        else:
            # Single hidden vector
            hidden_for_lambda = hidden_state.unsqueeze(0) if hidden_state.dim() == 1 else hidden_state
        
        # Ensure hidden_for_lambda has correct shape (1, hidden_dim)
