...
class AdaptiveLambda(nn.Module):
    ...
    """
    def __init__(self, input_dim=64):  # Changed: input_dim instead of hidden_dim
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim + 1, 64),  # +1 for timestep t
            nn.ReLU(),
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, 1),
    ...
    def forward(self, hidden_state, t):
        """
        hidden_state: (batch, input_dim) - from agent's policy network
        ...
        actual_dim = hidden_state.size(-1)
        ...
        # If dimension changed, we need to rebuild the network
        # For simplicity, just pass through existing net (assuming same dim)
        return self.net(x)
class Policy(nn.Module):
    ...
    def act(self, inputs, rnn_hxs, masks, deterministic=False):
        ...
        return value, action, action_log_probs, actor_features
