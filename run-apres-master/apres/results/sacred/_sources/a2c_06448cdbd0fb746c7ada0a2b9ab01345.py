...
class A2C:
    ...
    def update(
        ...
        lambda_val = max(0.1, 1.0 - t_normalized)  # decays from 1.0 to 0.4
        ...
        for oid in other_agent_ids:
            ...
            # kl=torch.abs(storages[oid].action_log_probs - logp)
            ...
            diff = torch.abs( storages[oid].action_log_probs - logp ) 
            # beta=torch.exp(- diff).detach()
            importance_sampling = torch.exp(- diff).detach()
