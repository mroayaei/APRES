...
class A2C:
    ...
    def update(
        ...
        #    , other_dist
            ...
            for i in range(num_steps):
                ...
                for j in range(num_processes):
                    ...
                    # matches_action = (storages[oid].actions[i][j] == self.storage.actions[i][j]).sum().item()
                    # measure2=matches_action/storages[oid].actions.view(-1, action_shape).numel() 
                    # matches_reward = (storages[oid].rewards == self.storage.rewards[i][j]).sum().item()
                    # measure3=matches_reward/storages[oid].rewards.view(-1, reward_shape).numel()
                    ...
                    # measure1=ham
                    # final_measure=(measure1+measure2+measure3)/3
        ...
        # (
        #     (1-seac_coef)*policy_loss
        #     + (1-seac_coef)*value_loss_coef * value_loss
        #     - entropy_coef * dist_entropy
        ...
        # for name, param in self.model.named_parameters():
        #     if param.grad is not None:
        #                 # Only apply sharing if this layer actually received contributions
        #         if received_contributions[name]:
        #             param.grad.copy_(alpha * weighted_grads[name] + (1 - alpha) * param.grad)
                # If no contributions received, keep the original gradient unchanged
                # param.grad.copy_(alpha*weighted_grads[name]+(1-alpha)*param.grad)
        ...
        # newwww 
        # return {
        #     "policy_loss": (1-seac_coef)*policy_loss.item(),
        #     "value_loss": (1-seac_coef)*value_loss_coef * value_loss.item(),
        #     "dist_entropy": entropy_coef * dist_entropy.item(),
        #     "importance_sampling":  (my_coef * importance_sampling).mean().item(),
