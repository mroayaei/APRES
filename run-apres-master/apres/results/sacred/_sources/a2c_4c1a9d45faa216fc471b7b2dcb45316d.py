...
class A2C:
    ...
    def update(
        ...
        my_obs = self.storage.obs[:-1].reshape(
            -1, *obs_shape
        )

        my_hidden = self.storage.recurrent_hidden_states[0].reshape(
            -1,
            self.model.recurrent_hidden_state_size
        )

        my_masks = self.storage.masks[:-1].reshape(
            -1,
            1
        )
        my_dist = self.model.get_distribution(
            my_obs,
            my_hidden,
            my_masks
        )
        ...
        for oid in other_agent_ids:
           ...
           # ===============================
            ...
            other_dist = agents[oid].model.get_distribution(
                my_obs,
                my_hidden,
                my_masks
            )
        ...
        # K = min(2, len(kl_values))   # change K here
        ...
        # )[:K]
            ...
            seac_value_loss += (pair_lambda*
                importance_sampling * other_advantage.pow(2)
            ).mean()
            seac_policy_loss += (pair_lambda*
