...
class A2C:
    ...
    def __init__(
        ...
        print(f"[DEBUG] Agent {agent_id} hidden_dim: {hidden_dim}")
        # ===== ADAPTIVE LAMBDA NETWORK (λ) - NEW =====
        # AdaptiveLambda(input_dim=hidden_dim) matches MLPBase hidden size
        self.adaptive_lambda = AdaptiveLambda(AdaptiveLambda(input_dim=hidden_dim))
    ...
    def update(
        ...
        print(f"[DEBUG] hidden_state shape: {hidden_state.shape}, dim: {hidden_state.dim()}")
        ...
        # ============================================================
        # FIXED: Extract hidden state for adaptive lambda
        # Calculate total batch size
        total_batch_size = num_steps_storage * num_processes_storage
        print(f"[DEBUG] total_batch_size: {total_batch_size}")

        # hidden_state from evaluate_actions is (total_batch_size, hidden_dim)
        # We want to take the first sample (index 0) for the hidden state
        if hidden_state.dim() == 2:
            # Expected shape: (total_batch_size, hidden_dim)
            # Take the first sample (index 0) as representative
            hidden_for_lambda = hidden_state[0:1, :]  # (1, hidden_dim)
        elif hidden_state.dim() == 1:
            # Single hidden vector
            hidden_for_lambda = hidden_state.unsqueeze(0)  # (1, hidden_dim)
        else:
            # Fallback: reshape to 2D and take first sample
            hidden_state_2d = hidden_state.view(-1, hidden_state.size(-1))
            hidden_for_lambda = hidden_state_2d[0:1, :]

        print(f"[DEBUG] hidden_for_lambda shape: {hidden_for_lambda.shape}")

        # Ensure shape is (1, hidden_dim)
        if hidden_for_lambda.dim() == 1:
            hidden_for_lambda = hidden_for_lambda.unsqueeze(0)
        # ============================================================
        ...
        if use_attention_weighting and len(other_agent_ids) > 0:
            for oid in other_agent_ids:
                ...
                # Handle case where other agent has no observations
                if other_obs_flat.size(0) == 0:
                    all_weights.append((oid, 0.5))  # neutral weight
                    continue
                
                # Sample observations from other agent
        ...
        selected_agents = all_weights[:min(top_k, len(all_weights))]  # list of (oid, weight)
        
        # ============================================================
        # STEP 7: Compute adaptive lambda (λ) - when to trust others
        # ============================================================
        
        # Pass hidden_for_lambda directly (should be (1, hidden_dim))
        # adaptive_lambda = self.adaptive_lambda(hidden_for_lambda, t_normalized).item()
        self.adaptive_lambda = AdaptiveLambda(input_dim=hidden_dim)  # ← CORRECT 
        print(f"[DEBUG] adaptive_lambda: {adaptive_lambda}")
        # ============================================================
